...
function calculerMoitieNuit(maghrib: string, fajr: string): string {
    ...
    return Math.floor(milieu / 60) % 24 + ':' + (milieu % 60).toString().padStart(2, '0')
}
function calculerDernierTiers(maghrib: string, fajr: string): string {
    ...
    return Math.floor(debut / 60) % 24 + ':' + (debut % 60).toString().padStart(2, '0')
}
...
export default function Prieres() {
    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: colors.fondCreme }} edges={[]}>
            ...
            {/* Header bleu */}
            <View style={{ backgroundColor: colors.bleu, paddingHorizontal: spacing.xl, paddingTop: 60, paddingBottom: spacing.xl, alignItems: 'center' }}>
                <Text style={{ fontFamily: typography.fontFamily.bold, fontSize: typography.size.xs, letterSpacing: 2, color: colors.or, textTransform: 'uppercase', marginBottom: 4 }}>
                    Horaires
                </Text>
                ...
                {ville ? (
                    <Text style={{ fontFamily: typography.fontFamily.regular, fontSize: typography.size.sm, color: 'rgba(255,255,255,0.55)', marginTop: 4 }}>
                        {ville}
                    </Text>
                ) : null}
                {dateHijri ? (
                    <View style={{ backgroundColor: 'rgba(217,172,42,0.15)', borderWidth: 1, borderColor: 'rgba(217,172,42,0.4)', borderRadius: radius.full, paddingHorizontal: spacing.lg, paddingVertical: spacing.xs, marginTop: spacing.sm }}>
                        <Text style={{ fontFamily: typography.fontFamily.medium, fontSize: typography.size.base, color: colors.or }}>
                            {dateHijri}
                        </Text>
                    </View>
                ) : null}
            ...
            {loading ? (
                ...
                <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: spacing.xl }}>
                    ...
                    <Text style={{ fontFamily: typography.fontFamily.regular, color: '#c00', textAlign: 'center' }}>{erreur}</Text>
                </View>
            ) : (
                <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: spacing.xl, paddingBottom: 160 }}>
                    {prochaine && (
                        <View style={{ backgroundColor: 'white', borderRadius: 20, borderWidth: 2, borderColor: colors.or, padding: spacing.xl, alignItems: 'center', marginBottom: spacing.xl, shadowColor: colors.or, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.12, shadowRadius: 24, elevation: 4 }}>
                            <View style={{ width: TAILLE_SVG, height: TAILLE_SVG, alignItems: 'center', justifyContent: 'center', marginBottom: spacing.md }}>
                                <Svg width={TAILLE_SVG} height={TAILLE_SVG} style={{ position: 'absolute', transform: [{ rotate: '-90deg' }] }}>
                                    ...
                                    <Circle cx={TAILLE_SVG / 2} cy={TAILLE_SVG / 2} r={RAYON} fill="none" stroke={colors.or} strokeWidth={10} strokeLinecap="round" strokeDasharray={CIRCONF} strokeDashoffset={dashOffset} />
                                </Svg>
                                <View style={{ alignItems: 'center' }}>
                                    <Text style={{ fontFamily: typography.fontFamily.regular, fontSize: typography.size.xs, color: '#aaa', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 4 }}>Prochaine Prière</Text>
                                    <Text style={{ fontFamily: typography.fontFamily.bold, fontSize: typography.size.xl, color: colors.bleu, marginBottom: 2 }}>{prochaine.nom}</Text>
                                    <Text style={{ fontFamily: typography.fontFamily.bold, fontSize: typography.size['2xl'], color: colors.or }}>{prochaine.heure}</Text>
                                </View>
                            </View>
                            <View style={{ backgroundColor: colors.fondCreme, borderRadius: radius.full, paddingHorizontal: spacing.lg, paddingVertical: spacing.xs, borderWidth: 1, borderColor: colors.bordure }}>
                                <Text style={{ fontFamily: typography.fontFamily.medium, fontSize: typography.size.sm, color: colors.bleu }}>
                    ...
                    <View style={{ gap: spacing.sm }}>
                        {horaires.map(p => {
                            ...
                            return (
                                <View key={p.cle} style={{ backgroundColor: estProchaine ? colors.bleu : 'white', borderWidth: 1, borderStyle: estTahajjud ? 'dashed' : 'solid', borderColor: estProchaine ? colors.bleu : estTahajjud ? colors.or : colors.bordure, borderRadius: radius.lg, paddingHorizontal: spacing.lg, paddingVertical: spacing.md, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', opacity: estPassee && !estTahajjud ? 0.45 : 1 }}>
                                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                        <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: estProchaine ? colors.or : estTahajjud ? colors.or : '#ccc', marginRight: spacing.md }} />
                                        <View>
                                            <Text style={{ fontFamily: estProchaine ? typography.fontFamily.bold : typography.fontFamily.medium, fontSize: typography.size.md, color: estProchaine ? 'white' : estPassee && !estTahajjud ? '#aaa' : colors.texte }}>{p.nom}</Text>
                                            {p.cle === 'Tahajjud' ? <Text style={{ fontFamily: typography.fontFamily.regular, fontSize: typography.size.xs, color: colors.orFonce, marginTop: 2 }}>Tahajjud — Prière de la nuit</Text>
                                                : p.cle === 'MoitieNuit' ? <Text style={{ fontFamily: typography.fontFamily.regular, fontSize: typography.size.xs, color: colors.orFonce, marginTop: 2 }}>Fin de l'heure du Isha</Text>
                                                    : null}
                                        </View>
                                    </View>
                                    <Text style={{ fontFamily: typography.fontFamily.bold, fontSize: typography.size.lg, color: estProchaine ? colors.or : estPassee && !estTahajjud ? '#ccc' : estTahajjud ? colors.orFonce : colors.bleu }}>{p.heure}</Text>
                                </View>
                            )
                        })}
                    </View>
