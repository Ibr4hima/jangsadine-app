...
function IcoCoran({ size = 16, color = colors.bleu }: IcoProps) {
  // Calligraphie « القرآن الكريم » (PNG RGBA transparent) teintée.
  // Double couche superposée pour épaissir les traits fins de la
  // calligraphie et les rapprocher visuellement des SVG remplis des
  // autres tuiles.
  const src = require('../../assets/icons/quran.png')
  const s = { width: size, height: size }
  return (
    <View style={[s, { alignItems: 'center', justifyContent: 'center' }]}>
      <Image source={src} style={[s, { tintColor: color, position: 'absolute' }]} resizeMode="contain" />
      <Image source={src} style={[s, { tintColor: color, position: 'absolute', opacity: 0.7 }]} resizeMode="contain" />
    </View>
  )
}
