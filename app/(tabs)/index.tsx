...
function IcoCoran({ size = 16, color = colors.bleu }: IcoProps) {
  // Calligraphie « القرآن الكريم » (PNG transparent) teintée à la couleur
  // de l'icône. Légèrement agrandie (×1.3) : la calligraphie est dense, il
  // lui faut un peu plus de place que les glyphes en trait des autres tuiles.
  const s = Math.round(size * 1.3)
  return (
    <Image
      source={require('../../assets/icons/quran.png')}
      style={{ width: s, height: s, tintColor: color }}
      resizeMode="contain"
    />
  )
}
